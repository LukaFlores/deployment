    ...
    for symbol in barsets:
        ...
        for bar in barsets[symbol]:
            ...
            if len(recent_closes) >= 50 and date.today().isoformat() == bar.t.date().isoformat():
                ...
                mom_12 = tulipy.mom(numpy.array(recent_closes), 12)[-1]
            else:
                sma_20, sma_50, rsi_14, mom_12 = None, None, None, None

            cursor.execute("""
                INSERT INTO stock_price (stock_id, date, open, high, low, close, volume, sma_20, sma_50, rsi_14, mom_12)
                VALUES (?, ?, ?, ?, ?, ?, ?,?,?,?,?)
            """, (stock_id, bar.t.date(), bar.o, bar.h, bar.l, bar.c, bar.v, sma_20, sma_50, rsi_14, mom_12))
